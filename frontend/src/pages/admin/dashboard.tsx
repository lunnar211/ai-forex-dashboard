...
export default function AdminDashboard() {
  ...
  // Auto-refresh activity every 30 seconds when on activity tab
  useEffect(() => {
    if (!token || activeTab !== 'activity') return;
          ...
          <div className="bg-[#1e293b] border border-[#334155] rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#334155]">
              <div>
                ...
                <p className="text-xs text-[#475569] mt-0.5">Auto-refreshes every 30 seconds</p>
              </div>
              <div className="flex items-center gap-2">
                {/* Filter buttons */}
                {(['all', 'login', 'register'] as const).map((f) => (
                  <button
                    key={f}
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-[#0f172a] border border-[#334155] hover:border-blue-500 text-[#94a3b8] hover:text-blue-400 text-xs rounded-lg transition-colors disabled:opacity-50"
                >
                  ↻ Refresh
                </button>
              </div>
            ...
            {loadingActivity ? (
              ...
              <div className="px-6 py-12 text-center text-[#475569]">
                ...
                <p className="text-xs mt-1">Activity will appear here when users log in or register.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#334155]">
                      <th className="text-left px-6 py-3 text-[#475569] text-xs font-semibold uppercase tracking-wider">Time</th>
                      <th className="text-left px-6 py-3 text-[#475569] text-xs font-semibold uppercase tracking-wider">Action</th>
                  ...
                  <tbody className="divide-y divide-[#0f172a]">
                    {activity.map((item) => (
                      <tr key={item.id} className="hover:bg-[#0f172a]/40 transition-colors">
                        <td className="px-6 py-3 text-[#475569] text-xs whitespace-nowrap">
                        ...
                        <td className="px-6 py-3 text-white text-xs">
                          {item.name || <span className="text-[#475569]">—</span>}
                        </td>
                        <td className="px-6 py-3 text-[#94a3b8] text-xs">{item.email}</td>
